...
export class CalendarWeekViewCurrentTimeMarkerComponent implements OnInit, OnChanges {
  ...
  */
  ...
  ngOnInit(): void {
      this.columnDate$ = new BehaviorSubject<Date>(this.columnDate);
      this.marker$ =
        this.zone.onStable.pipe(
        switchMap(() => interval(60 * 1000)),
        startWith(0),
        switchMapTo(this.columnDate$),
        map((columnDate_:Date) => {
          const startOfDay = this.dateAdapter.setMinutes(
            this.dateAdapter.setHours(columnDate_, this.dayStartHour),
            this.dayStartMinute
          );
          const endOfDay = this.dateAdapter.setMinutes(
            this.dateAdapter.setHours(columnDate_, this.dayEndHour),
            this.dayEndMinute
          );
          const hourHeightModifier =
            (this.hourSegments * this.hourSegmentHeight) / 60;
          ...
          return {
            isVisible:
              this.dateAdapter.isSameDay(columnDate_, now) &&
              now >= startOfDay &&
              now <= endOfDay,
            top:
              this.dateAdapter.differenceInMinutes(now, startOfDay) *
